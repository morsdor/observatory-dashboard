...
const TableRow = memo<RowProps>(({ index, style, data }) => {
    ...
    // Debug: Log when rows are actually rendered (only visible ones)
    React.useEffect(() => {
        console.log(`🎯 Virtualized row ${index} rendered in DOM`)
    }, [index])
    ...
    return (
        <div
            ...
            className={`
        flex border-b border-gray-100 hover:bg-gray-50 cursor-pointer relative
        ${isSelected ? 'bg-blue-50 border-blue-200' : ''}
      `}
            onClick={(event) => onRowClick(item, event)}
            onDoubleClick={() => onRowDoubleClick(item)}
        >
            {/* Virtual row indicator */}
            <div className="absolute left-0 top-0 w-1 h-full bg-green-400 opacity-30" title="Virtualized row - only visible rows are in DOM" />
            ...
            {/* Row index indicator for debugging */}
            <div className="absolute right-2 top-1 text-xs text-gray-400 bg-white px-1 rounded">
                #{index}
            </div>
        </div>
    )
...
export const VirtualizedTable = memo<VirtualizedTableProps>(function VirtualizedTable({
    ...
    const listHeight = height - HEADER_HEIGHT - 80
    const visibleRowCount = Math.ceil(listHeight / ROW_HEIGHT)
    const renderedRowCount = Math.min(visibleRowCount + 10, sortedData.length) // +10 for overscan
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900">
                            Virtualized Data Table ({sortedData.length.toLocaleString()} rows)
                        </h3>
                        <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                            <span className="bg-green-100 text-green-800 px-2 py-1 rounded">
                                Only {renderedRowCount} DOM elements
                            </span>
                            <span>Visible: ~{visibleRowCount} rows</span>
                            <span>Performance: {((renderedRowCount / sortedData.length) * 100).toFixed(1)}% DOM usage</span>
                        </div>
                    </div>
